...
def parse_epoch(value: Any) -> int:
    """Parse a timestamp-like value into epoch seconds (int).

    Returns 0 on failure. Handles numeric (s or ms), formatted strings and ISO.
    """
    if value is None:
        return 0
    if isinstance(value, (int, float)):
...
def convert(input_path: str, output_path: str, overwrite: bool = False) -> None:
    ...
    msgs = []
    # First, build a list with epoch values so we can compute time diffs reliably
    for m in data.get('messages', []) or []:
        ...
        epoch = parse_epoch(raw_ts)
        # If parse_epoch failed (0), try to fall back to parse_timestamp for human readable
        formatted_ts = parse_timestamp(raw_ts) if epoch == 0 else datetime.utcfromtimestamp(epoch).strftime("%Y-%m-%d %H:%M:%S")
    ...
    # Sort by epoch (oldest first) and compute 'time' difference (seconds) between consecutive messages
    msgs = sorted(msgs, key=lambda x: x.get('_epoch', 0))
    prev_epoch = None
    for m in msgs:
        epoch = m.get('_epoch', 0) or 0
        if prev_epoch is None:
            m['time'] = 0
        else:
            # If either epoch is zero (unknown), set time to 0
            if prev_epoch == 0 or epoch == 0:
                m['time'] = 0
            else:
                m['time'] = int(epoch - prev_epoch)
        prev_epoch = epoch
        # remove helper key
        m.pop('_epoch', None)
